(function (g, v) {
  ...
  function ea() {
    if (null == L) {
      L = {};
      v('#region').children().each(function () {
        var a = v(this),
          b = a.val();
        if (b) {
          L[b] = a.text();
    ...
    v.get('http://m.agar.io/info', function (a) {
      for (var b in a.regions) {
        v('#region option[value="' + b + '"]').text(L[b] + ' (' + a.regions[b].numPlayers + ' players)');
      }
    }, 'json');
  }
  ...
  function ya(a) {
    ...
    var b = Math.random(),
      c = 1;
    aa = false;
    var f = a.getUint16(c, true),
      c = c + 2,
      d;
    for (d = 0; d < f; ++d) {
      var e = y[a.getUint32(c, true)],
  ...
  function Aa() {
    ...
    a = Math.pow(Math.min(64 / a, 1), 0.4) * Math.max(p / 965, n / 1920);
    s = (9 * s + a) / 10;
  }
  ...
  function U() {
    ...
    if (0 != I) {
      ...
      d.globalAlpha = 0.2;
      d.fillStyle = '#000000';
      d.fillRect(10, p - 10 - 24 - 10, b + 10, 34);
      d.globalAlpha = 1;
      d.drawImage(c, 15, p - 10 - 24 - 5);
    };
    Da();
    a = +new Date() - a;
    if (a > 1000 / 60) {
  ...
  if ('agar.io' != g.location.hostname && 'localhost' != g.location.hostname && '10.10.2.13' != g.location.hostname) {
    ...
    oa.prototype = {
      ...
      setName: function (a) {
        if (this.name = a) {
          if (null == this.nameCache) {
            this.nameCache = new N(this.getNameSize(), '#FFFFFF', true, '#000000');
          } else {
      ...
      shouldRender: function () {
        return this.x + this.size + 40 < w - n / 2 / s || this.y + this.size + 40 < x - p / 2 / s || this.x - this.size - 40 > w + n / 2 / s || this.y - this.size - 40 > x + p / 2 / s ? false : true;
      },
      draw: function () {
        if (this.shouldRender()) {
          ...
          if (null != b && 0 < b.width && !a) {
            d.save();
            d.clip();
            d.drawImage(b, this.x - this.size, this.y - this.size, 2 * this.size, 2 * this.size);
            d.restore();
          };
          if (da || 15 < this.size) {
          ...
          d.globalAlpha = 1;
          if (null != b && 0 < b.width && a) {
            d.drawImage(b, this.x - 2 * this.size, this.y - 2 * this.size, 4 * this.size, 4 * this.size);
          };
          a = -1 != m.indexOf(this);
          b = ~~this.y;
          if ((O || a) && this.name && this.nameCache) {
            c = this.nameCache.render();
            d.drawImage(c, ~~this.x - ~~(c.width / 2), b - ~~(c.height / 2));
            b += c.height / 2 + 4;
          };
          if (sa && a) {
            if (null == this.sizeCache) {
              this.sizeCache = new N(this.getNameSize() / 2, '#FFFFFF', true, '#000000');
            };
